...
Our first task is to aggregate all the data so that we can do some EDA and start creating the prediciton models. As was mentioned before, we need to create three datasets for each goal's EDA and three datasets for each goal's prediction model.
...
The first step we took in our EDA for MVP winners was to look at how much better they are at each facet of the game than their peers. The method we used to quantify this was to use percentile rankings (99th percentile means 99% of players got less of a certain stat than out MVP winner). In order to calculate the percentile of all MVPs, we counted how many total players there were and then counted how many total players scored lower in each stat than our MVPS. Then, by dividing the total number lower by the total count, we obntained our percentile.
